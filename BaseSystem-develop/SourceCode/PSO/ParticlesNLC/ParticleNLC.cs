...
namespace PSO.ParticlesNLC
{
    ...
    public abstract class ParticleNLC : IComparable<ParticleNLC>, IParticleNLC
    {
        ...
        public void UpdateVelocityAndPosition(int[] bestPositionOfSwarm, IConstraints nonLinearConstraints)
        {
            ...
            for (var i = 0; i < localVelocity.Length; i++)
            {
                ...
                if (newVelocity > vmax)
                    newVelocity = (int) vmax;
                //if (newVelocity < -vmax)
                //    newVelocity = -vmax;
                if (newVelocity < vmin)
                    newVelocity = (int) vmin;
        ...
        private void RecalculateVelocityAndPosition(IConstraints constraints)
        {
            ...
            while (!isFeasible && nTrials < MaximumNumberOfTrials)
            {
                ...
                var newPosition = new int[Position.Length];
                ...
                for (var positionIndex = 0; positionIndex < Position.Length; positionIndex++)
                {
                    ...
                    randomNumber = Swarm.Sampler.NextDouble();
                }
                //Atualiza partícula
            ...
            if (Swarm.UseFootholds)
            {
                ...
                while (isFeasible && tries < 10)
                {
                    ...
                    var newPositionFoothold = new int[Position.Length];
